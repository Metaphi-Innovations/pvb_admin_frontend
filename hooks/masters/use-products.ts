...
export function useProducts(params: MasterListKeyParams) {
  return useQuery({
    ...
    queryFn: ({ signal }) =>
      ProductListService.list({ ...toListParams(params), signal }),
  });
}
...
export function useProductPreviewNumber() {
  return useQuery({
    queryKey: [...masterKeys.products.all(), "preview-number"],
...
export function useCreateProduct() {
  ...
  return useMutation({
    mutationFn: (payload: ProductCreatePayload) =>
      ProductListService.create(payload),
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: masterKeys.products.lists(),
      });
    },
  });
}
...
export function useExportProducts() {
  return useMutation({
    mutationFn: (params: ProductExportParams) =>
      ProductListService.export(params),
  });
}
