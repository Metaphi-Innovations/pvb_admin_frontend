/**
 * GSTR-1 B2C Invoices report — unregistered-customer sales invoices.
 */
...
export type B2cStatusFilter = "all" | "valid" | "exception";
export type B2cGstRateFilter = "all" | "5" | "12" | "18" | "28";

export interface B2cInvoiceFilters extends GstDashboardFilters {
  customerId: string | string[];
  gstRate: B2cGstRateFilter;
  invoiceNo: string;
  status: B2cStatusFilter;
}

export interface B2cRateBreakup {
  ratePct: number;
  taxableValue: number;
  cgst: number;
  sgst: number;
...
export interface B2cLineDetail {
  id: string;
  productName: string;
...
export type B2cInvoiceStatus = "valid" | "exception";

export interface B2cInvoiceRow {
  invoiceId: number;
  invoiceDate: string;
  invoiceNo: string;
  ...
  rateBreakups: B2cRateBreakup[];
  lineDetails: B2cLineDetail[];
  branch: string;
  warehouse: string;
  voucherNo: string;
...
export interface B2cSummaryTotals {
...
export interface B2cInvoiceReport {
  rows: B2cInvoiceRow[];
  totals: B2cSummaryTotals;
  hasData: boolean;
}

const B2C_EXCLUDED_EXCEPTION_CODES = new Set<GstExceptionCode>([
...
function detectB2cSalesInvoiceExceptions(
  inv: InvoiceRecord,
  duplicateNos: Set<string>,
): GstExceptionIssue[] {
  return detectSalesInvoiceExceptions(inv, duplicateNos).filter(
    (ex) => !B2C_EXCLUDED_EXCEPTION_CODES.has(ex.code),
  );
}

function resolveCustomerType(inv: InvoiceRecord): string {
  if (inv.customerId != null) {
    const customer = loadCustomers().find((c) => c.id === inv.customerId);
    if (customer?.customerType?.trim()) return customer.customerType.trim();
    if (customer && !customer.gstApplicable) return "Unregistered";
  }
  if (inv.customerGstCategory?.trim()) {
    return inv.customerGstCategory.trim();
  }
  return "Unregistered";
}

function resolveGstRegistrationStatus(inv: InvoiceRecord): string {
  if (inv.customerId != null) {
    const customer = loadCustomers().find((c) => c.id === inv.customerId);
    if (customer?.gstApplicable) return "GST Registered";
  }
  return "Unregistered";
}

function matchesBaseFilters(inv: InvoiceRecord, filters: B2cInvoiceFilters): boolean {
...
function buildRateBreakups(inv: InvoiceRecord, interstate: boolean): B2cRateBreakup[] {
...
function buildLineDetails(inv: InvoiceRecord, interstate: boolean): B2cLineDetail[] {
...
function formatGstRateLabel(breakups: B2cRateBreakup[]): string {
...
function invoiceToB2cRow(inv: InvoiceRecord, duplicateNos: Set<string>): B2cInvoiceRow {
  ...
  const exceptions = detectB2cSalesInvoiceExceptions(inv, duplicateNos);
  ...
  return {
    ...
    customerType: resolveCustomerType(inv),
    gstRegistrationStatus: resolveGstRegistrationStatus(inv),
...
function matchesB2cFilters(row: B2cInvoiceRow, filters: B2cInvoiceFilters): boolean {
...
function buildTotals(rows: B2cInvoiceRow[]): B2cSummaryTotals {
  return {
...
export function buildB2cInvoiceReport(filters: B2cInvoiceFilters): B2cInvoiceReport {
  const duplicateNos = buildDuplicateInvoiceNumbers();
  const rows: B2cInvoiceRow[] = [];

  for (const inv of loadInvoices()) {
    if (!isB2cEligibleInvoice(inv)) continue;
    if (!matchesBaseFilters(inv, filters)) continue;
    const row = invoiceToB2cRow(inv, duplicateNos);
    if (!matchesB2cFilters(row, filters)) continue;
    rows.push(row);
  }
...
export function getB2cCustomerOptions(): { id: string; name: string }[] {
...
export interface B2cExceptionFixAction {
  label: string;
  href: string;
}

export function resolveB2cExceptionFixAction(
  code: GstExceptionCode,
  inv: Pick<InvoiceRecord, "id" | "customerId" | "lineItems">,
): B2cExceptionFixAction | null {
  switch (code) {
    ...
    case "missing_hsn": {
      const line = inv.lineItems.find((l) => l.qty > 0 && !l.hsn?.trim());
      if (line?.productId) {
        return { label: "Fix Product", href: `/masters/products/${line.productId}/edit` };
      }
      return { label: "Fix HSN Master", href: "/masters/hsn" };
    }
...
export function parseB2cFiltersFromSearch(
  search: string,
  defaults: B2cInvoiceFilters,
): B2cInvoiceFilters {
  const params = new URLSearchParams(search);
  const gstRate = params.get("gstRate") as B2cGstRateFilter | null;
  const status = params.get("status") as B2cStatusFilter | null;
  const validRates: B2cGstRateFilter[] = ["all", "5", "12", "18", "28"];
  const validStatus: B2cStatusFilter[] = ["all", "valid", "exception"];
...
export function buildB2cFilterQuery(filters: B2cInvoiceFilters): string {
...
export {
  getGstDashboardBranchOptions as getB2cBranchOptions,
  getGstDashboardWarehouseOptions as getB2cWarehouseOptions,
};
