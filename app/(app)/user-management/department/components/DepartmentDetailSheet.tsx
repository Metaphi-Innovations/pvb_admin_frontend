...
import {
  Building2, Calendar, Clock, FileText, Activity, Edit2,
} from "lucide-react";
    ...
    <Sheet open={open} onOpenChange={onClose}>
      <SheetContent>
        ...
        <SheetHeader>
          <div className="flex items-start gap-3 pr-8">
            ...
            <div className="flex-1 min-w-0">
              <SheetTitle className="truncate">{dept.name}</SheetTitle>
            </div>
        ...
        <SheetBody className="space-y-6">
          ...
          {/* Basic Info */}
          <div>
            <SectionHeading icon={FileText} label="Department Details" />
            <div className="bg-muted/20 rounded-xl border border-border/50 px-3">
              <InfoRow label="Department Name"  value={dept.name} />
              <InfoRow label="Status"           value={dept.status.charAt(0).toUpperCase() + dept.status.slice(1)} />
              {dept.remarks && <InfoRow label="Remarks" value={dept.remarks} />}
            </div>
          </div>

          {/* History / Audit Trail */}
          <div>
            <SectionHeading icon={Activity} label="History & Audit" />
