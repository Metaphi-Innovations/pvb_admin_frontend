...
import {
  ArrowLeft, Save, Check, ChevronsUpDown, AlertCircle, ChevronDown,
...
import {
  ...
  loadRoles, saveRoles, nextRoleId, todayStr,
...
function ApprovalStepRow({
  step, index, total, allRoles, existingIds,
  onRemove, onMoveUp, onMoveDown,
}: {
  step: ApprovalStep; index: number; total: number; allRoles: Role[];
  existingIds: number[]; onRemove: () => void; onMoveUp: () => void; onMoveDown: () => void;
}) {
...
function AddStepPopover({
  ...
  const candidates = allRoles.filter(r =>
    ...
    (geoLevel === "None"
      ? true  // non-field roles can pick any approver
        ...
        <div className="max-h-[200px] overflow-y-auto py-1">
          {candidates.length === 0 ? (
            <p className="px-3 py-4 text-xs text-muted-foreground text-center">
              {geoLevel !== "None"
                ? "No roles at a higher geography level"
      ...
      <div className="flex flex-col" style={{ height: "calc(100vh - 104px)" }}>
        ...
        <div className="flex-shrink-0 bg-white border-b border-border px-5 py-3 flex items-center gap-3 z-10">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-muted-foreground">User Management → Roles → New Role</p>
          </div>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6 bg-muted/10">
            <div className="space-y-6">
              ...
              {/* Section 3 — Approval Chain (optional) */}
              <SectionCard icon={CheckSquare} title="Approval Chain" subtitle="Define who approves for this role — skip if no approval is required" optional>
                <div className="space-y-4">
                  <div className="flex items-start gap-2.5 bg-muted/30 border border-border rounded-lg px-3.5 py-3 max-w-2xl">
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      Approval is <strong>not mandatory</strong>. You can add, remove, or reconfigure the approval chain at any time.
                      Leave empty if no approval workflow is needed for this role.
                    </p>
                  </div>
                    ...
                    <div className="space-y-2 max-w-md">
                      {approvalChain.map((step, i) => (
                        <ApprovalStepRow
                          ...
                          allRoles={allRoles} existingIds={approvalChain.map(s => s.roleId)}
          ...
          <div className="w-64 flex-shrink-0 border-l border-border bg-white flex flex-col overflow-y-auto">
              ...
              <div className="space-y-2 text-[11px]">
                <div className="flex items-start gap-2">
                  ...
                  <span className="font-medium text-foreground truncate">
                    {form.roleName.trim() || <span className="text-muted-foreground italic">Not set</span>}
                  </span>
                </div>
                <div className="flex items-start gap-2">
                  <span className="text-muted-foreground w-20 flex-shrink-0">Dept</span>
                  <span className="font-medium text-foreground">
                    {selectedDept?.name ?? <span className="text-muted-foreground italic">Not set</span>}
                  </span>
                </div>
                <div className="flex items-start gap-2">
                  <span className="text-muted-foreground w-20 flex-shrink-0">Geo</span>
                  <span className="font-medium text-foreground">
                    {form.geoLevel === "None" ? <span className="text-muted-foreground italic">None</span> : form.geoLevel}
                  </span>
                </div>
                <div className="flex items-start gap-2">
                  <span className="text-muted-foreground w-20 flex-shrink-0">Approval</span>
                  <span className="font-medium text-foreground">
                    {approvalChain.length === 0
                      ? <span className="text-muted-foreground italic">Not set</span>
                      : `${approvalChain.length} step${approvalChain.length > 1 ? "s" : ""}`}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="p-5 space-y-3">
              <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Completion</p>
              <div className="w-full h-1.5 bg-muted rounded-full overflow-hidden">
                <div className="h-full bg-brand-600 rounded-full transition-all duration-300" style={{ width: `${progress}%` }} />
              </div>
              <p className="text-[11px] text-muted-foreground">{progress}% filled</p>
              <div className="space-y-1.5">
                {[
                  { label: "Role Name", done: filled[0] },
                  { label: "Department", done: filled[1] },
                  { label: "Description", done: filled[2] },
                  { label: "Geo Level", done: filled[3], optional: true },
                  { label: "Parent Role", done: filled[4], optional: true },
                  { label: "Approval Chain", done: filled[5], optional: true },
                ].map(item => (
                  <div key={item.label} className="flex items-center gap-2">
                    <div className={cn("w-3.5 h-3.5 rounded-full flex items-center justify-center flex-shrink-0",
                      item.done ? "bg-emerald-500" : item.optional ? "bg-muted border border-border" : "bg-muted border border-border")}>
                      {item.done && <Check className="w-2 h-2 text-white" />}
                    </div>
                    <span className={cn("text-[11px]", item.done ? "text-foreground" : "text-muted-foreground")}>
                      {item.label}
                      {item.optional && !item.done && <span className="ml-1 text-[10px] italic">(optional)</span>}
                    </span>
                  </div>
                ))}
              </div>
