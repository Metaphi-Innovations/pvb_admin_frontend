    ...
    if (search) {
      ...
      result = result.filter(
        (row) =>
          ...
          (row.salesReturnNo ?? "").toLowerCase().includes(q) ||
  ...
  const actions: ActionItemConfig<GrnRecord>[] = [
    {
      ...
      action: "view",
      icon: Eye,
      onClick: (row) => router.push(`/warehouse/grn/sales-return/${row.id}`),
    },
    {
      label: "Perform QC",
      action: "qc",
