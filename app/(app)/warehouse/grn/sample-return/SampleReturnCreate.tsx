...
export function SampleReturnCreate() {
  ...
  useEffect(() => {
    ...
    const count = existing.filter((g) => g.sourceType === "sample_return").length;
    setGrnNo(`GRN-SMP-${String(count + 1).padStart(4, "0")}`);
  }, []);

  // 2. Fetch all pending sample returns
  const availableReturns = useMemo(() => {
    ...
    const processedNos = new Set(
      existingGrns
        .filter((g) => g.sourceType === "sample_return")
        .map((g) => g.sampleReturnNo)
        .filter(Boolean)
    );
  ...
  // Selected Sample Return Record Details
  ...
  useEffect(() => {
    ...
    const initialLines: LineInputState[] = activeReturn.products.map((p) => {
      ...
      return {
        ...
        batchNo: p.batchNo || "B-SMP-UNKNOWN",
  ...
  const handleSave = () => {
    if (!selectedWarehouse) {
      setError("Please select a sample return with a valid destination warehouse.");
      return;
    }
    if (!selectedReturnId || !activeReturn) {
      setError("Please select a sample return record to receive.");
      return;
    }
    ...
    const newGrn: GrnRecord = {
      ...
      sampleReturnNo: activeReturn.returnNumber,
    ...
    onGrnCreated(newGrn);
    router.push("/warehouse/grn/sample-return");
  };

  return (
    <FormContainer
      title="Create Sample Return GRN"
      description="Record receipt of returned stock from sample orders. Batch details and case sizes are read-only."
      onBack={() => router.push("/warehouse/grn/sample-return")}
      onCancel={() => router.push("/warehouse/grn/sample-return")}
      actions={
        <Button
    ...
    >
      <div className="space-y-6">
        ...
        <SectionCard
          ...
          description="Select sample return record first. Destination warehouse will automatically populate and lock."
        >
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            <Field label="Select Sample Return" required>
              <AutocompleteSelect
                ...
                placeholder="Select sample return…"
