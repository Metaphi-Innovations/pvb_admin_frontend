  ...
  useEffect(() => {
    ...
    if (window.location.hash === "#follow-up-history") {
      const t = window.setTimeout(() => {
        document.getElementById("follow-up-history")?.scrollIntoView({ behavior: "smooth", block: "start" });
      }, 300);
      return () => window.clearTimeout(t);
    }
    if (window.location.hash === "#vendor-invoice") {
      const t = window.setTimeout(() => {
        document.getElementById("vendor-invoice")?.scrollIntoView({ behavior: "smooth", block: "start" });
      }, 300);
      return () => window.clearTimeout(t);
    }
    if (window.location.hash === "#three-way-match") {
      const t = window.setTimeout(() => {
        document.getElementById("three-way-match")?.scrollIntoView({ behavior: "smooth", block: "start" });
      }, 300);
      return () => window.clearTimeout(t);
    }
