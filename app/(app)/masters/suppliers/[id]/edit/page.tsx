...
function supplierToForm(supplier: SupplierListRecord): VendorFormValues {
  ...
  return {
    ...
    paymentType: (() => {
      const raw = String(supplier.bankAccounts?.[0]?.payment_type || "credit").toLowerCase();
      if (raw.startsWith("immediate")) return "immediate";
      if (raw.startsWith("advance")) return "advance";
      return "credit";
    })(),
    creditDays: (() => {
      const paymentType = String(supplier.bankAccounts?.[0]?.payment_type || "credit").toLowerCase();
      if (paymentType.startsWith("immediate") || paymentType.startsWith("advance")) {
        return "";
      }
      const days = supplier.bankAccounts?.[0]?.credit_days;
      return days != null && days !== "" ? String(days) : "";
    })(),
    advancePercentage: (() => {
      const paymentType = String(supplier.bankAccounts?.[0]?.payment_type || "").toLowerCase();
      if (!paymentType.startsWith("advance")) return "";
      const advance = supplier.bankAccounts?.[0]?.advance;
      return advance != null && advance !== "" ? String(advance) : "";
    })(),
  };
}
