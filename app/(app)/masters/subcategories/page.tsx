...
export default function SubCategoryMasterPage() {
	...
	const [sortKey, setSortKey] = useState<SortKey>("subCategoryCode");
	...
	const filtered = useMemo(() => {
		return records
			...
			.filter((r) =>
				filterStatus.length ? filterStatus.includes(r.status) : true,
			)
			.sort((a, b) => {
			...
			<div className='space-y-5'>
				<div className='flex items-start justify-between'>
					...
					<div className='flex items-center gap-2'>
						...
						<Link href='/masters/subcategories/add'>
							<Button
								size='sm'
				...
				<div className='flex flex-wrap items-center gap-2'>
					<div className='relative flex-1 min-w-[220px] max-w-sm'>
						...
						<Input
							...
							placeholder='Search sub category code, name, category...'
					...
					<Popover>
						...
						<PopoverContent
							align='start'
							className='w-56 p-0 bg-white border shadow-lg border-border'
						>
										...
										>
											...
											<td className='px-3 py-2.5'>
												<button
													...
													className='inline-flex items-center gap-1.5 rounded-full px-0.5 py-0.5 transition-opacity focus:outline-none hover:opacity-90'
											...
											<td className='sticky right-0 z-20 w-[80px] min-w-[80px] px-3 py-2.5 bg-white border-l border-border shadow-[-8px_0_12px_-12px_rgba(0,0,0,0.25)]'>
												<DropdownMenu>
													...
													<DropdownMenuContent
														align='end'
														className='w-32 bg-white border shadow-lg border-border'
													>
														<DropdownMenuItem asChild>
															<Link
																href={`/masters/subcategories/${row.id}`}
														...
														<DropdownMenuItem asChild>
															<Link
																href={`/masters/subcategories/${row.id}/edit`}
																className='flex items-center gap-2'
															>
						...
						<p className='text-[11px] text-muted-foreground'>
							{filtered.length === 0 ? (
								...
								<>
									...
									<span className='font-medium text-foreground'>{filtered.length}</span> sub categories
								</>
							)}
						</p>
						<div className='flex items-center gap-2'>
