...
import { Edit2, FileText, UserCheck, UserX } from "lucide-react";
  ...
  return (
    <div className="space-y-4">
              ...
              <div key={item.id} className="rounded-xl border border-border bg-white p-3 shadow-sm">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex min-w-0 items-start gap-3">
                    ...
                    <div className="min-w-0">
                      <p className="truncate text-xs font-medium text-foreground">{item.title || item.name}</p>
                      <p className="truncate text-[11px] text-muted-foreground">{item.size || item.fileType || item.mediaKind || item.type}</p>
                    </div>
                  </div>
                ...
                <div className="mt-3 flex items-center justify-end gap-2">
                  <Button
                    ...
                    onClick={() =>
                      item.type === "media" && (item.mediaKind === "image" || item.kind === "image") && url
      ...
      <Dialog open={!!selectedItem} onOpenChange={(open) => !open && setSelectedItem(null)}>
        <DialogContent className="max-w-3xl rounded-xl border border-border bg-white p-4 shadow-lg">
          <DialogHeader className="border-b border-border/50 pb-2">
            <DialogTitle className="truncate text-sm font-semibold text-foreground">{selectedItem?.name}</DialogTitle>
          </DialogHeader>
          <div className="flex items-center justify-center overflow-hidden rounded-lg bg-muted/5 py-4">
            <img src={selectedItem?.src} alt={selectedItem?.name} className="max-h-[70vh] max-w-full rounded-md object-contain" />
          </div>
        </DialogContent>
      </Dialog>
    </div>
  ...
  if (!product) {
    return (
      <div className="py-16 text-center">
        ...
        <Link href="/masters/products" className="mt-2 inline-block text-xs text-brand-600 hover:underline">
          Back to listing
        </Link>
      </div>
    );
    ...
    <FormContainer
      ...
      actions={
        <div className="flex items-center gap-2">
          ...
          >
            {product.status === "active" ? <UserX className="h-3.5 w-3.5" /> : <UserCheck className="h-3.5 w-3.5" />}
      ...
      <div className="w-full space-y-5">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
          <DetailCard title="Basic Details">
            <InfoRow label="Product Code" value={product.productId} mono />
            ...
            <InfoRow label="Status" value={<StatusPill status={product.status} />} />
          </DetailCard>

          <DetailCard title="Tax & Pricing Details">
          ...
          <DetailCard title="Packaging & Stock Details">
