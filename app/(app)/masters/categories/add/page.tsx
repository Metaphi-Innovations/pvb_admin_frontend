...
import { generateCategoryCode, loadCategories, nextCategoryId, saveCategories, todayStr, type Category } from "../category-data";
...
export default function AddCategoryPage() {
  ...
  const [categoryCode, setCategoryCode] = useState("CAT-001");

  useEffect(() => {
    const items = loadCategories();
    setCategoryCode(generateCategoryCode(items));
    setForm((prev) => ({ ...prev, categoryCode: generateCategoryCode(items) }));
  }, []);
  ...
  const persist = () => {
    ...
    saveCategories([...items, record]);
    setToast({ msg: "Category added successfully", type: "success" });
    setTimeout(() => router.push("/masters/categories"), 900);
  };
      ...
      <div className="flex flex-col" style={{ minHeight: "calc(100vh - 104px)" }}>
        <div className="sticky top-0 z-10 flex flex-shrink-0 items-center gap-2.5 border-b border-border bg-white px-4 py-2">
          ...
          <div className="min-w-0 flex-1">
            <h2 className="text-sm font-semibold leading-none text-foreground">Add Category</h2>
            <p className="mt-0.5 text-[11px] text-muted-foreground">Masters → Category Master → Add</p>
          </div>
          <span className="rounded bg-brand-50 px-2 py-0.5 font-mono text-[11px] font-semibold text-brand-700">{categoryCode}</span>
