...
const PER_PAGE = 10;
const VIEW_DISTRIBUTOR_STORAGE_KEY = "distributor:view-id";

export default function DistributorPage() {
  const router = useRouter();
  const pathname = usePathname();
  const [distributors] = useState<Distributor[]>(SEED);
  ...
  useEffect(() => {
    if (pathname !== "/database/distributor/view") {
  ...
  const viewDistributorRecords = useMemo(() => {
    if (!viewDistributor) return filtered;
    return filtered.some((distributor) => distributor.id === viewDistributor.id) ? filtered : distributors;
  }, [filtered, distributors, viewDistributor]);
  const currentViewDistributorIndex = viewDistributor
    ? viewDistributorRecords.findIndex((distributor) => distributor.id === viewDistributor.id)
    : -1;
  ...
  const handleStepViewDistributor = (direction: -1 | 1) => {
    ...
    const nextDistributor = viewDistributorRecords[currentViewDistributorIndex + direction];
  ...
  return (
    <AppLayout>
      ...
      <DistributorViewDialog
        ...
        onPrevious={() => handleStepViewDistributor(-1)}
        onNext={() => handleStepViewDistributor(1)}
        canPrevious={currentViewDistributorIndex > 0}
        canNext={currentViewDistributorIndex >= 0 && currentViewDistributorIndex < viewDistributorRecords.length - 1}
      />
    </AppLayout>
  );
}
