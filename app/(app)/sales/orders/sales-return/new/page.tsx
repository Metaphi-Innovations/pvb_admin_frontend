...
export default function NewSalesReturnPage() {
  ...
  const dispatchOptions = useMemo(() => {
    ...
    return deliveredDispatches
      .filter((d) => getSalesOrderNo(d) === selectedSalesOrderNo)
      .map((d) => ({
        value: d.id,
  ...
  useEffect(() => {
    ...
    const record = deliveredDispatches.find((d) => d.id === selectedDispatchId);
  ...
  const handleSave = () => {
    ...
    const totalAmount = productsToReturn.reduce((sum, p) => sum + p.lineAmount, 0);
        ...
        {dispatch ? (
          <>
            ...
            <SalesReturnProductForm
              ...
              onCheckedChange={(sku, checked, defaultQty) => {
