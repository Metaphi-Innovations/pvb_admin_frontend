...
function StatusSelect({
  ...
  const [open, setOpen] = useState(false);
  const options = ORDER_STATUS_OPTIONS.filter(o => allowedStatuses.includes(o.value));
      ...
      <PopoverContent className="w-[--radix-popover-trigger-width] p-1">
        {options.map(opt => (
          ...
          >
            ...
            {value === opt.value && <Check className="w-3.5 h-3.5 text-brand-600" />}
          </button>
        ))}
      </PopoverContent>
    </Popover>
...
interface SalesOrderFormProps {
  ...
  auditInfo?: { createdBy: string; createdDate: string; updatedBy: string; updatedDate: string };
}

export function validateSalesOrderForm(form: SalesOrderFormValues): Record<string, string> {
...
export function validateSplitOrderForm(
  ...
  for (const line of form.lineItems) {
    ...
    const parentLine = originalOrder.lineItems.find(l => l.id === line.splitSourceLineId);
...
function ImportFromOriginalPopover({
  ...
  const [open, setOpen] = useState(false);
  const sourceLines = originalOrder.lineItems.filter(l => l.productId && l.quantity > 0);

  const importLine = (parentLine: SalesOrderLineItem) => {
    const product = parentLine.productId ? getProductById(parentLine.productId) : undefined;
    const qty = 1;
        ...
        {mode === "edit" && auditInfo && (
          <div className="bg-muted/30 rounded-xl p-3 space-y-2 text-[11px]">
            ...
            <div className="grid grid-cols-2 gap-y-1.5 gap-x-4">
              <div><span className="text-muted-foreground">Created By</span><p className="font-medium">{auditInfo.createdBy}</p></div>
              <div><span className="text-muted-foreground">Created Date</span><p className="font-medium">{auditInfo.createdDate}</p></div>
              <div><span className="text-muted-foreground">Updated By</span><p className="font-medium">{auditInfo.updatedBy}</p></div>
              <div><span className="text-muted-foreground">Updated Date</span><p className="font-medium">{auditInfo.updatedDate}</p></div>
            </div>
          </div>
        )}
      </div>
