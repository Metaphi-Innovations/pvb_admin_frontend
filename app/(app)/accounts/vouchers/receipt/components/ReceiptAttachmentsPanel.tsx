...
import type { ReceiptAttachmentMeta, ReceiptPendingFile } from "@/types/receipt-voucher.types";
import {
  RECEIPT_ATTACHMENT_ACCEPT,
 ...
 * Persisted metadata vs pending File objects are kept separate.
 * Does not alter shared VoucherAttachmentSection behavior.
 */
export function ReceiptAttachmentsPanel({
  persisted,
  pending,
  readOnly,
  onAddFiles,
  ...
  persisted: ReceiptAttachmentMeta[];
  pending: ReceiptPendingFile[];
  readOnly?: boolean;
  ...
  return (
    <div className="space-y-1 min-w-0">
      ...
      <p className="text-[11px] text-muted-foreground">
        Max {RECEIPT_ATTACHMENT_MAX_FILES} files, 10MB each. JPEG, PNG, GIF, PDF, DOC,
        DOCX, XLS, XLSX, CSV.
      </p>
      ...
      {!readOnly ? (
        ...
        >
          ...
          <input
            ...
            className="hidden"
            multiple
            accept={RECEIPT_ATTACHMENT_ACCEPT}
            onChange={(e) => {
          ...
          {pending.map((item) => (
            ...
            >
              ...
              <span className="text-muted-foreground whitespace-nowrap">
                {formatReceiptFileSize(item.file.size)} · pending
              </span>
              {item.previewUrl ? (
                <button
                  type="button"
