...
import type { PaymentFormState } from "../payment-voucher-utils";
import { computePaymentPreview, toMoneyNumber } from "../payment-voucher-utils";

type PreviewLine = { ledger: string; debit: number; credit: number };

export function PaymentAccountingPreview({
  form,
  partyLedgerName,
  defaultOpen = false,
}: {
  ...
  const lines = useMemo(() => {
    ...
    const isDirectCustomerRefund =
      ...
      form.allocations.filter((a) => a.selected && toMoneyNumber(a.allocated_amount) > 0)
        .length === 0;

    const debitName =
      form.party_kind === "OTHER_LEDGER" || isDirectCustomerRefund
        ? form.other_ledger_name ||
          (isDirectCustomerRefund ? "Refund / Adjustment Ledger" : "Other Ledger")
        : partyLedgerName ||
          (form.party_kind === "SUPPLIER" ? "Supplier" : "Customer");

    if (preview.gross > 0) {
      result.push({ ledger: debitName, debit: preview.gross, credit: 0 });
    }
    ...
    if (preview.netBank > 0) {
      result.push({ ledger: bankName, debit: 0, credit: preview.netBank });
    }
