...
import type { JournalAttachmentMeta, JournalPendingFile } from "@/types/journal-voucher.types";
import {
...
export function JournalAttachmentsPanel({
  ...
  persisted: JournalAttachmentMeta[];
  pending: JournalPendingFile[];
  readOnly?: boolean;
  ...
  return (
    <div className="space-y-1 min-w-0">
      ...
      <p className="text-[11px] text-muted-foreground">
        Max {JOURNAL_ATTACHMENT_MAX_FILES} files, 10MB each. JPEG, PNG, GIF, PDF, DOC,
        DOCX, XLS, XLSX, CSV.
      </p>
          ...
          {pending.map((item) => (
            ...
            >
              ...
              <span className="text-muted-foreground whitespace-nowrap">
                {formatJournalFileSize(item.file.size)} · pending
              </span>
              {item.previewUrl ? (
                <button
                  type="button"
