import { ACCOUNTS_CURRENT_USER, ACCOUNTS_INVOICE_ADMIN } from "@/lib/accounts/config";
import { getCustomersForTransactionDropdown, type Customer } from "@/app/(app)/masters/customers/customer-data";
...
export type InvoiceCreditStatus = "no_credit" | "partially_credited" | "fully_credited";
...
export function calcLineAmounts(line: Pick<InvoiceLineItem, "qty" | "unitPrice" | "discountPct" | "taxPct">) {
...
export function derivePaymentStatus(grandTotal: number, amountReceived: number): InvoicePaymentStatus {
  if (amountReceived <= 0) return "unpaid";
  if (amountReceived >= grandTotal) return "paid";
  return "partially_paid";
}

export function deriveCreditStatus(grandTotal: number, amountCredited: number): InvoiceCreditStatus {
...
export function normalizeInvoice(rec: InvoiceRecord): InvoiceRecord {
  ...
  const balanceAmount = Math.max(0, Math.round((totals.grandTotal - amountReceived) * 100) / 100);
  const amountCredited = lines.reduce((s, l) => s + (l.creditedAmount ?? 0), 0);
  const balanceCreditAllowed = Math.max(0, Math.round((totals.grandTotal - amountCredited) * 100) / 100);
  const paymentStatus =
    rec.invoiceStatus === "cancelled" ? rec.paymentStatus : derivePaymentStatus(totals.grandTotal, amountReceived);
...
export function applyProductToInvoiceLine(line: InvoiceLineItem, product: InvoiceProductOption): InvoiceLineItem {
...
export function customerToInvoiceFields(c: Customer) {
  const mobile = c.countryCode && c.mobile ? `${c.countryCode} ${c.mobile}` : c.mobile;
  const address = [c.address, c.districtName, c.stateName, c.pincode].filter(Boolean).join(", ");
  return {
    customerId: c.id,
...
const SEED: InvoiceRecord[] = [
  {
    ...
    attachments: [],
    activity: [{ at: "2026-05-28T09:00:00.000Z", action: "created", by: "Admin", detail: "Invoice created as sent" }],
    createdBy: "Admin",
    updatedBy: "Admin",
  ...
  },
  {
    ...
    collections: [],
    attachments: [],
    activity: [{ at: "2026-06-02T08:00:00.000Z", action: "created", by: "Admin", detail: "Saved as draft" }],
    createdBy: "Admin",
    updatedBy: "Admin",
...
export function saveInvoices(records: InvoiceRecord[]): void {
  ...
  localStorage.setItem(STORAGE_KEY, JSON.stringify(records.map(normalizeInvoice)));
}
...
export function tabMatchesInvoice(rec: InvoiceRecord, tab: string): boolean {
  ...
  if (tab === "partially_paid") return rec.paymentStatus === "partially_paid" && rec.invoiceStatus !== "cancelled";
  if (tab === "paid") return rec.paymentStatus === "paid" && rec.invoiceStatus !== "cancelled";
  if (tab === "cancelled") return rec.invoiceStatus === "cancelled";
  return true;
}

export function filterInvoices(records: InvoiceRecord[], filters: InvoiceListFilters): InvoiceRecord[] {
  let r = records.map(normalizeInvoice).filter((e) => tabMatchesInvoice(e, filters.tab));
...
export function computeInvoiceTabCounts(records: InvoiceRecord[]): Record<string, number> {
  ...
  return {
    ...
    partially_paid: n.filter((r) => tabMatchesInvoice(r, "partially_paid")).length,
...
export function canEditInvoice(rec: InvoiceRecord): boolean {
  ...
  if (rec.invoiceStatus === "cancelled" || rec.paymentStatus === "paid") return false;
  return rec.invoiceStatus === "draft" || rec.invoiceStatus === "sent";
}

export function getInvoiceRowActions(rec: InvoiceRecord): (
  | "view"
  | "edit"
  | "pdf"
  | "email"
  ...
  if (rec.invoiceStatus === "sent" && rec.paymentStatus !== "paid") actions.push("email");
  if (rec.invoiceStatus !== "cancelled" && rec.balanceAmount > 0) actions.push("receive");
  if (rec.invoiceStatus !== "cancelled" && rec.paymentStatus !== "paid") actions.push("cancel");
  return actions;
}
    ...
    activity: [
      {
        ...
        action: "created",
        by: ACCOUNTS_CURRENT_USER,
        detail: input.invoiceStatus === "draft" ? "Invoice saved as draft" : "Invoice created and sent",
      },
    ],
...
export function updateInvoice(id: number, input: InvoiceFormInput): InvoiceRecord {
  const all = loadInvoices();
  const idx = all.findIndex((r) => r.id === id);
  if (idx < 0) throw new Error("Invoice not found");
  const cur = normalizeInvoice(all[idx]);
  if (!canEditInvoice(cur)) throw new Error("Invoice cannot be edited in current status.");
...
export function markInvoiceSent(id: number): InvoiceRecord {
  ...
  if (idx < 0) throw new Error("Invoice not found");
  const cur = all[idx];
  if (cur.invoiceStatus === "cancelled") throw new Error("Cancelled invoice cannot be sent.");
  const updated = normalizeInvoice({
    ...cur,
    invoiceStatus: "sent",
...
export function receiveInvoicePayment(
  ...
  if (idx < 0) throw new Error("Invoice not found");
  const cur = normalizeInvoice(all[idx]);
  if (cur.invoiceStatus === "cancelled") throw new Error("Cannot collect on cancelled invoice.");
  if (payload.amount <= 0) throw new Error("Amount must be greater than zero.");
  if (payload.amount > cur.balanceAmount) throw new Error(`Amount exceeds balance (${cur.balanceAmount}).`);
...
export function lookupSalesInvoiceForCredit(invoiceId: number): SalesInvoiceCreditLookup | null {
...
export function reconcileInvoiceCredits(
  invoiceId: number,
  lineCredits: { lineId: string; creditedQty: number; creditedAmount: number }[],
): void {
  const all = loadInvoices();
