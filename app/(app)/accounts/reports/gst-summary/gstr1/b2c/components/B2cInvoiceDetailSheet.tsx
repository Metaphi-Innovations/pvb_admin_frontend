...
export function B2cInvoiceDetailSheet({
  row,
  open,
  onClose,
}: {
  row: B2cInvoiceRow | null;
  open: boolean;
  onClose: () => void;
}) {
  if (!row) return null;
        ...
        <SheetBody className="space-y-4">
          {row.exceptions.length > 0 && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 space-y-2">
              ...
              {row.exceptions.map((ex, i) => {
                const fix =
                  sourceInv && resolveB2cExceptionFixAction(ex.code, sourceInv);
                return (
          ...
          <div>
            ...
            <div className="bg-muted/20 rounded-xl border border-border/50 px-3">
              ...
              <InfoRow label="Customer Type" value={row.customerType} />
              <InfoRow label="GST Registration" value={row.gstRegistrationStatus} />
