...
import {
  buildB2cFilterQuery,
  buildB2cInvoiceReport,
  getB2cBranchOptions,
  getB2cCustomerOptions,
  getB2cWarehouseOptions,
  parseB2cFiltersFromSearch,
  resolveB2cExceptionFixAction,
  type B2cGstRateFilter,
  type B2cInvoiceFilters,
  type B2cInvoiceRow,
  type B2cStatusFilter,
} from "@/lib/accounts/gstr1-b2c-compute";
import { loadInvoices } from "@/app/(app)/accounts/invoices/invoices-data";
import { exportB2cInvoicesToExcel, exportB2cInvoicesToPdf } from "./gstr1-b2c-export";
import { B2cInvoiceDetailSheet } from "./components/B2cInvoiceDetailSheet";
import { useDebouncedValue } from "../../../pl/pl-hooks";

const PLACEHOLDER_DATE = "2025-04-01";

const GST_RATE_OPTIONS: { value: B2cGstRateFilter; label: string }[] = [
...
const STATUS_OPTIONS: { value: B2cStatusFilter; label: string }[] = [
  { value: "all", label: "All" },
...
function RateBreakupRows({ row }: { row: B2cInvoiceRow }) {
  return (
    <>
...
export default function B2cInvoicesPageClient() {
  ...
  const [gstRate, setGstRate] = useState<B2cGstRateFilter>("all");
  const [invoiceNo, setInvoiceNo] = useState("");
  const [status, setStatus] = useState<B2cStatusFilter>("all");
  const [exporting, setExporting] = useState(false);
  const [expandedIds, setExpandedIds] = useState<Set<number>>(new Set());
  const [viewRow, setViewRow] = useState<B2cInvoiceRow | null>(null);
  ...
  useEffect(() => {
    ...
    if (!qs) return;
    const { from, to, fyId } = defaultFyDateRange();
    const parsed = parseB2cFiltersFromSearch(qs, {
  ...
  const b2cFilters = useMemo((): B2cInvoiceFilters => ({
    financialYearId,
    dateFrom,
    dateTo,
    branch,
  ...
  const branchOptions = useMemo(
    () => (mounted ? getB2cBranchOptions() : REPORT_BRANCH_OPTIONS),
    [mounted, b2cFilters],
  );
  const warehouseOptions = useMemo(
    () => (mounted ? getB2cWarehouseOptions() : []),
    [mounted, b2cFilters],
  );
  const customerOptions = useMemo(
    () => (mounted ? getB2cCustomerOptions() : []),
    [mounted, b2cFilters],
  );
  ...
  const report = useMemo(() => {
    ...
    return buildB2cInvoiceReport(b2cFilters);
  }, [mounted, datesReady, b2cFilters]);
  ...
  const backHref = `/accounts/reports/gst-summary/gstr1?${buildB2cFilterQuery({
    ...b2cFilters,
    customerId: [],
    gstRate: "all",
    invoiceNo: "",
  ...
  const handleExportExcel = async () => {
    setExporting(true);
    try {
      await exportB2cInvoicesToExcel(report.rows, report.totals, exportMeta);
    } finally {
      setExporting(false);
    }
  };
  ...
  const handleExportPdf = () => {
    exportB2cInvoicesToPdf(report.rows, report.totals, exportMeta);
  };
  ...
  return (
    <AccountsPageShell
      breadcrumbs={accountsBreadcrumb("Reports", "GST Summary", "B2C Invoices")}
      title="B2C Invoices"
      description="Sales invoices issued to unregistered customers for GSTR-1."
      hideDescription
      layout="split"
        ...
        <AccountsListingTableCard className="flex-1 min-h-0 flex flex-col">
          {!mounted || !datesReady ? (
            <div className="flex items-center justify-center py-6 text-xs text-muted-foreground">
              Loading B2C invoices…
            </div>
          ) : !report.hasData ? (
            <div className="accounts-table-empty py-4 text-center">
              <p className="text-sm text-muted-foreground">
                No B2C invoices match the selected filters.
              </p>
              {hasFilters && (
                <button
                  type="button"
              ...
              <AccountsTable minWidth={1400}>
                <AccountsTableHead>
                  <AccountsTableHeadRow>
                    ...
                    <AccountsTableHeadCell>Customer Type</AccountsTableHeadCell>
                  ...
                  {report.rows.map((row) => {
                    ...
                    const primaryFix =
                      row.status === "exception" && sourceInv && row.exceptions[0]
                        ? resolveB2cExceptionFixAction(row.exceptions[0].code, sourceInv)
                        : null;
                    ...
                    return (
                      <Fragment key={row.invoiceId}>
                        ...
                        >
                          ...
                          <AccountsTableCell className="text-xs">{row.customerType}</AccountsTableCell>
