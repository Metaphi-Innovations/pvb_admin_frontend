...
import { buildGstr2aReconciliationRows } from "@/lib/accounts/gst-report-service";
...
export default function Gstr2aPageClient() {
  ...
  const rows = useMemo(() => {
    ...
    return buildGstr2aReconciliationRows(filters);
    ...
    try {
      exportGstTabularReport(
        ...
        rows.map((row) => ({
          ...
          booksTaxableValue: row.booksTaxableValue,
          portalTaxableValue: row.portalTaxableValue,
  ...
  return (
    <AccountsPageShell
      ...
      description="Reconcile purchase ITC with imported GSTR-2A data (demo)."
      hideDescription
      layout="split"
              ...
              <AccountsTable minWidth={1100}>
                <AccountsTableHead>
                  <AccountsTableHeadRow>
                    ...
                    <AccountsTableHeadCell align="right" className="text-xs font-semibold">Books Taxable</AccountsTableHeadCell>
                    <AccountsTableHeadCell align="right" className="text-xs font-semibold">Portal Taxable</AccountsTableHeadCell>
                  ...
                  {rows.map((row) => (
                    <AccountsTableRow key={row.id}>
                      ...
                      <AccountsTableCell align="right" money className={cn("text-xs", MONEY_AMOUNT_CLASS)}>
                        {formatMoney(row.booksTaxableValue)}
                      </AccountsTableCell>
                      <AccountsTableCell align="right" money className={cn("text-xs", MONEY_AMOUNT_CLASS)}>
                        {formatMoney(row.portalTaxableValue)}
                      </AccountsTableCell>
                      <AccountsTableCell
                        align="right"
                        money
