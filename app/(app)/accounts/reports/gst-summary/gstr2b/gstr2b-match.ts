import { AMOUNT_TOLERANCE, type Gstr2bDocType, type Gstr2bMatchStatus } from "./gstr2b-report-types";
...
export function compareMatchedPair(
  ...
  status: Extract<
    Gstr2bMatchStatus,
    "itc_available" | "itc_not_available" | "partial_match" | "needs_review"
  >;
  remarks: string;
  ...
  if (!dateMismatch && taxableOk && gstOk) {
    if (portal.itcAvailable) {
      return {
        status: "itc_available",
        remarks: "ITC available — all key fields match",
        dateMismatch: false,
        taxableDifference,
        gstDifference,
      };
    ...
    return {
      ...
      remarks: "ITC not available as per GSTR-2B",
      dateMismatch: false,
      taxableDifference,
      gstDifference,
    };
