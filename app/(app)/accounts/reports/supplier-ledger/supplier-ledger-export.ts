...
const REPORT_NAME = "Supplier Ledger";
...
export async function exportSupplierLedgerToExcel(
  ...
  XLSX.utils.book_append_sheet(wb, sheet, "Supplier Ledger");

  const info = XLSX.utils.aoa_to_sheet([
    ["Company Name", COMPANY_NAME],
    ["Report Name", REPORT_NAME],
    ["Supplier Name", summary.supplierName],
    ["Supplier Code", summary.supplierCode],
    ["GSTIN", summary.gstin],
    ["PAN", summary.pan],
  ...
  XLSX.writeFile(wb, `Supplier_Ledger_${safeName}_${date}.xlsx`);
}
...
export function exportSupplierLedgerToPdf(
  ...
  const tableRows = rows
    ...
    <tr class="${r.kind !== "transaction" ? "summary-row" : ""}">
      <td>${r.date}</td>
      <td>${r.kind === "transaction" ? r.voucherNo : r.kind === "opening" ? "OB" : "—"}</td>
      <td>${r.voucherType}</td>
      <td>${r.particular}</td>
      <td>${r.narration}</td>
...
<head>
  <meta charset="utf-8" />
  <title>Supplier Ledger — ${summary.supplierName}</title>
  <style>
    ...
    th { background: #fff; font-size: 10px; text-transform: uppercase; position: sticky; top: 0; }
    .num { text-align: right; font-variant-numeric: tabular-nums; }
    .summary-row td { font-weight: 600; background: #fafafa; }
  </style>
</head>
<body>
  <h1>${summary.supplierName}</h1>
  <p class="meta">
    ${summary.supplierCode} · GSTIN: ${summary.gstin} · PAN: ${summary.pan}<br />
