...
export default function BankReconCompleteReviewPageClient({
  ...
  const breadcrumbs = [
    ...RECONCILIATION_BREADCRUMB,
    {
      label: account?.accountNickname ?? "Account",
      href: bankReconWorkspacePath(accountId),
    },
    { label: "Complete" },
  ];
    ...
    return (
      ...
      >
        ...
        <Button asChild size="sm" variant="outline" className={ACCOUNTS_ACTION_BUTTON_CLASS}>
          <Link href="/accounts/banking/reconciliation">Back to Bank Reconciliation</Link>
        </Button>
      </AccountsPageShell>
    );
  }
  ...
  return (
    <>
      <AccountsPageShell
        breadcrumbs={breadcrumbs}
        title="Complete Reconciliation"
        description={`${account.bankName} · ${account.accountNickname} · Review before completion`}
        layout="form"
              ...
              <>
                ...
                <Button
                  ...
                  className={cn(
                    ...
                    "gap-1 bg-brand-600 hover:bg-brand-700 text-white border-0",
                  )}
      ...
      >
        <div className="max-w-[1200px] space-y-3 pb-4">
          {!readOnly && (
            <div className="flex flex-wrap gap-2 items-end">
              <div className="space-y-1">
                ...
                <Input
                  ...
                  className="h-8 text-xs w-[140px] accounts-filter-control"
                />
              </div>
              <div className="space-y-1">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            <Section title="Bank & Period Details">
              <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
                ...
                <p className="font-medium">{review.bankName}</p>
                ...
                <p className="text-muted-foreground">Statement File</p>
                <p className="font-medium truncate">{review.statementFileName ?? "—"}</p>
                <p className="text-muted-foreground">Prepared By</p>
                <p className="font-medium">{session?.preparedBy ?? "—"}</p>
              </div>
            </Section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            <Section title="Book Balance Summary">
              <div className="space-y-1 text-xs tabular-nums">
                <div className="flex justify-between">
                  <span>Opening Book Balance</span>
                  <span>{formatMoney(review.bookOpeningBalance)}</span>
                </div>
                <div className="flex justify-between text-emerald-700">
                  <span>+ Book Deposits</span>
                  <span>{formatMoney(review.bookDeposits)}</span>
                </div>
                <div className="flex justify-between text-red-600">
                  <span>- Book Withdrawals</span>
                  <span>{formatMoney(review.bookWithdrawals)}</span>
                </div>
                <div className="flex justify-between font-semibold border-t border-border pt-1">
                  <span>Closing Book Balance</span>
                  <span>{formatMoney(review.bookClosingBalance)}</span>
                </div>
              </div>
            </Section>
            <Section title="Statement Balance Summary">
              <div className="space-y-1 text-xs tabular-nums">
                <div className="flex justify-between">
                  <span>Opening Statement Balance</span>
                  <span>{formatMoney(review.statementOpeningBalance)}</span>
                </div>
                <div className="flex justify-between text-emerald-700">
                  <span>+ Statement Deposits</span>
                  <span>{formatMoney(review.statementDeposits)}</span>
                </div>
                <div className="flex justify-between text-red-600">
                  <span>- Statement Withdrawals</span>
                  <span>{formatMoney(review.statementWithdrawals)}</span>
                </div>
                <div className="flex justify-between font-semibold border-t border-border pt-1">
                  <span>Closing Statement Balance</span>
                  <span>{formatMoney(review.statementClosingBalance)}</span>
                </div>
              </div>
            </Section>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
              <div className="space-y-1 tabular-nums">
                ...
                <div className="flex justify-between">
                  ...
                  <span>{formatMoney(review.formula.bookClosingBalance)}</span>
                </div>
                <div className="flex justify-between">
                  <span>+ Deposits in Transit</span>
                  <span>{formatMoney(review.formula.addDepositsInTransit)}</span>
                </div>
                <div className="flex justify-between">
                  <span>- Unpresented Cheques</span>
                  <span>{formatMoney(review.formula.lessUnpresentedCheques)}</span>
                </div>
                <div className="flex justify-between font-semibold border-t border-border pt-1">
                  <span>Adjusted Book Balance</span>
                  <span>{formatMoney(review.formula.adjustedBookBalance)}</span>
                </div>
              </div>
              <div className="space-y-1 tabular-nums">
                ...
                <div className="flex justify-between">
                  ...
                  <span>{formatMoney(review.formula.statementClosingBalance)}</span>
                </div>
                <div
                  className={cn(
                    "flex justify-between font-bold text-sm pt-2",
                    hasDiff ? "text-red-600" : "text-emerald-700",
                  )}
                >
                  <span>Final Difference</span>
                  <span>{formatMoney(review.formula.finalDifference)}</span>
                </div>
                {hasDiff && !canCompleteWithDifference() && (
                  <p className="text-[11px] text-red-600 mt-2 flex items-start gap-1">
                    <AlertTriangle className="w-3.5 h-3.5 shrink-0 mt-0.5" />
                    Reconciliation cannot be completed because the adjusted book balance does not
                    match the statement closing balance.
                  </p>
                )}
              </div>
            </div>
          ...
          {review.comparisonWithPrevious && (
            <Section title="Comparison with Previous Period">
              <div className="grid grid-cols-2 gap-2 text-xs">
                <p>
                  Previous Closing Book:{" "}
                  <strong>
                    {formatMoney(review.comparisonWithPrevious.previousClosingBookBalance ?? 0)}
                  </strong>
                </p>
                <p>
                  Current Opening Book:{" "}
                  <strong>
                    {formatMoney(review.comparisonWithPrevious.currentOpeningBookBalance)}
                  </strong>
                </p>
                {!review.comparisonWithPrevious.bookOpeningContinuityOk && (
                  <p className="col-span-2 text-red-600 font-medium">
                    Current opening book balance does not match previous completed closing balance.
                  </p>
                )}
              </div>
            </Section>
              ...
              <table className="accounts-table w-full">
                <thead className="bg-muted/40 sticky top-0">
                  <tr>
                    ...
                    <th className="text-left">Ageing</th>
                  ...
                  {review.outstandingBook.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="accounts-table-empty">
                        No outstanding book entries.
                      </td>
                    </tr>
                  ) : (
                    review.outstandingBook.map((o) => (
                      <tr key={o.bookTargetId} className="accounts-table-row">
                        <td>{o.bookDate}</td>
                        <td className="font-mono text-brand-700">{o.voucherNo}</td>
                        <td>{o.partyLedger}</td>
                        <td className="text-right tabular-nums">
                          {formatMoney(o.deposit || o.withdrawal)}
                        </td>
                        <td>{o.ageingDays}d</td>
                        <td>{o.reason}</td>
                      </tr>
                    ))
                  )}
              ...
              <table className="accounts-table w-full">
                ...
                <tbody>
                  {review.statementOnly.length === 0 ? (
                    <tr>
                      <td colSpan={4} className="accounts-table-empty">
                        No statement-only entries.
                      </td>
                    </tr>
                  ) : (
                    review.statementOnly.map((s) => (
                      <tr key={s.statementTransactionId} className="accounts-table-row">
                        <td>{s.statementDate}</td>
                        <td>{s.reference || s.narration.slice(0, 40)}</td>
                        <td className="text-right tabular-nums">
                          {formatMoney(s.deposit || s.withdrawal)}
                        </td>
                        <td>{s.requiredAction}</td>
                      </tr>
                    ))
                  )}
                </tbody>
          ...
          {session?.status === "Completed" || session?.status === "Completed with Difference" ? (
            <Section title="Completion Details">
              <div className="grid grid-cols-2 gap-2 text-xs">
                <p>
                  Completed By: <strong>{session.completedBy}</strong>
                </p>
                <p>
                  Completed On:{" "}
                  <strong>
                ...
                {session.differenceOverrideReason && (
                  <p className="col-span-2 text-amber-700">
                    Difference override: {session.differenceOverrideReason} (Approved by{" "}
                    {session.differenceApprovedBy})
                  </p>
                )}
              </div>
            </Section>
      ...
      <Dialog open={diffDialogOpen} onOpenChange={setDiffDialogOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              Difference: {formatMoney(review.formula.finalDifference)} — override requires reason
              and approval.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-xs">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button
                ...
                className={cn(
                  ACCOUNTS_ACTION_BUTTON_CLASS,
                  "bg-brand-600 hover:bg-brand-700 text-white border-0 gap-1",
                )}
                onClick={handleComplete}
              >
