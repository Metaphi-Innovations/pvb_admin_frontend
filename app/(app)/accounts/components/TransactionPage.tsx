...
export function TransactionPage({
  ...
  const visible = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      r = r.filter((x) => x.number.toLowerCase().includes(q) || x.party.toLowerCase().includes(q) || x.referenceNo.toLowerCase().includes(q));
    }
    return r.sort((a, b) => {
      ...
      <div className="max-w-[1400px] mx-auto space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button className="h-9 text-sm bg-brand-600 text-white" onClick={createDraft}>
        ...
        <div className="relative max-w-md">
          ...
          <Input value={search} onChange={(e) => setSearch(e.target.value)} className="h-9 pl-9 text-sm" placeholder={`Search ${title}`} />
        </div>
